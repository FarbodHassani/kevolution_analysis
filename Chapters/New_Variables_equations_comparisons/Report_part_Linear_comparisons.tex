...
\paragraph{ Comparing the initial condition from class and what is implemented in Gevolution}
...
Now lets try to do the same procedure but with Leap frog solver, this could be very good confirmation of Leap frog method, since we can check the error versus Euler method and look for possible bugs in the code!
 \paragraph{$\zeta$ equation}
  ...
  - 3 \mathcal{H}' _{n}) \pi_{n}+ 3 c_s^2 \Phi'_{n}  +c_s^2 \nabla^2 \pi_{n}
    \ee
    Since we need to have $\zeta_n$ at $\zeta'_n$ so we write $\zeta_{n} = \frac{\zeta_{n+1/2} + \zeta_{n-1/2} }{2}$ \\ 
    We get,
      \be
...
The best way to do it is first to check that $\zeta_n$ from the next order corrector is  near to original one (like the relative error $2\frac{\zeta_1-\zeta_2}{\zeta_1+\zeta_2}<0.01$, which gives less than 1$\%$ error).  So what we are doing to do is using the $\zeta_n$  from the one of the following formulas (or the average maybe?)
 ...
 \ee
  ...
  - 3 \mathcal{H}' _{n}) \pi_{n}+ 3 c_s^2 \Phi'_{n}  +c_s^2 \nabla^2 \pi_{n} \Bigg) \Bigg]
  \ee
  Then again we compute $\zeta_n$ by new $\zeta_{n+\frac{1}{2}}$ and $\zeta_{n-\frac{1}{2}}$ and check the error, then we continue the loop until we get the desired precision on the variable. \\
...
So the $\delta_{kess} = \frac{1+w}{c_s^2} \Big(- 3 \mathcal{H}c_s^2 \pi +\zeta     \Big ) $ which now we want to compare between Gevolution and mathematica solution for when this stress tensor does not contribute to the total stress tensor in Gevolution which is what we are able to catch in mathematica!
 ...
 \end{figure}
  Now what happens if we let also the stress tensor of kessence contribute in the total stress energy tensor? That is a important check which shows that the stress tensor is added in Gevolution correctly, since from previous experience we know that if we add it with wrong coefficients the particles speed up and go over cpus!
  
  
 \paragraph{Leap-frog method: Full equation}
Here we try to compare mathematica with Gevolution in scalar fields solution  when the gravitational potentials are turned on and kessence does not source gravity yet
 \paragraph{Mathematica vs Gevolution}
We try to compare stress tensor and fields in Gevolution with Mathematica solution, to just make sure everything is under the control,
\subsection{Comparing class, mathematica and Gevolution with initial condition from class for mathematica}
Giving initial condition from Class, comparing Class with Mathematica and Gevolution solution, to just quantify the effect of non-linear $\Psi$ in Gevolution and etc.

\subsection{Checking the limit $c_s^2 \to 0$ that we get kessence density as matter density }
-For the limit $c_s^2 \to 0$ check we get matter power from Gevolution compared with class!\\
We have checked that we get such a behaviour in class, do we get in Gevolution too?\\
This is a consistency check but not so much necessary for our purposes!
\section{Non-linear contrubution}
Here we write the full equations and try to solve them numerically! \\
We need to measure the effect of non linear terms on scalar field and etc\\
If we get wrong result we need to solve the non-linear terms in mathematica and compare with Gevolution directly?!
