...
\section{Non-linear contrubution (Bad way to do it)}
...
We chose to update $\zeta$ at half steps since then we need its derivative at integer steps and make the computation simplified. Moreover first $\pi$ is updated to have it at step $n+1$, then $\zeta$ is updated to get the values at $n+3/2$ and from taking the average of the two steps $(n+1/2)$ and $(n+3/2)$ we get $\zeta$ at step n which is synchronized with particles stress tensor at step n.
  \be
  \zeta_{n+\frac{3}{2}}=\zeta_{n+\frac{1}{2}} + \zeta'_{n+1} \Delta \tau \label{eqzeta}
  \ee
  ...
  -   (1-c_s^2)  \Psi_{n+1} {\nabla^2 \pi_{n+1}}
  %////////////////
  -3 c_s^2 \mathcal{H}_{n+1} (1+w)\pi_{n+1} {\nabla^2 \pi_{n+1} }
      %////////////////
        +  (1-c_s^2)  { (\zeta_{n+1} + \Psi_{n+1}) } \nabla^2 {\pi_{n+1} }
                                       \nonumber
   \\
    &
    %//////////////// 
   ...
   %//////////////// 
        +(2 c_s^2-1) {\nabla  \Psi_{n+1} . \nabla \pi_{n+1} }  
   %//////////////// 
 -\frac{\mathcal{H}_{n+1}} {2 } \Big(2+3w+c_s^2  \Big){\nabla  \pi_{n+1} . \nabla \pi_{n+1}} 
    %//////////////// 
     +2   (1-c_s^2){\nabla  \pi_{n+1} . {  \nabla {  (\zeta_{n+1} + \Psi_{n+1})   }}}   
    \end{align}
    
    Since we need to have $\zeta_n$ at $\zeta'_{n+1}$ so we write $\zeta_{n+1} = \frac{\zeta_{n+3/2} + \zeta_{n+1/2} }{2}$ \\ 
    We get,
      \begin{align}
  ...
  -3 c_s^2 \mathcal{H}_{n+1} (1+w)\pi_{n+1} {\nabla^2 \pi_{n+1} }
      %////////////////
        +  (1-c_s^2)  { \Big( \frac{\zeta_{n+3/2} + \zeta_{n+1/2} }{2}+ \Psi_{n+1} \Big) } \nabla^2 {\pi_{n+1} }
                       \nonumber
   \\
    &
    %//////////////// 
 ...
 -\frac{\mathcal{H}_{n+1}} {2 } \Big(2+3w+c_s^2  \Big){\nabla  \pi_{n+1} . \nabla \pi_{n+1}} 
    %//////////////// 
    \nonumber \\ &
     +2   (1-c_s^2){\nabla  \pi_{n+1} . {  \nabla {  \Big(\ \frac{\zeta_{n+3/2} + \zeta_{n+1/2} }{2}+ \Psi_{n+1} \Big)   }}}    \Bigg]
    \end{align}
As one can see the term $ \nabla   \Big( \frac{\zeta_{n+3/2} + \zeta_{n+1/2} }{2}+ \Psi_{n+1} \Big)  $ is somehow problematic, since we cannot factorize the $\zeta_{n+3/2}$ and $\zeta_{n+1/2}$ simply. Here we use the predictor-corrector method to solve this equation. \\
 ...
 -\frac{\mathcal{H}_{n+1}} {2 } \Big(2+3w+c_s^2  \Big){\nabla  \pi_{n+1} . \nabla \pi_{n+1}} 
    %//////////////// 
    \nonumber \\ &
     +2   (1-c_s^2)\nabla  \pi_{n+1} . {  \nabla {  \Big({\zeta_{n+1/2} + \Psi_{n+1} \Big)   } }}    \Bigg]
    \end{align}
    %%%%%%%%%%%%%%
    %%%%%%%%%%%%%%
    %%%%%%%%%%%%%%
    Factorizing $\zeta_{n+3/2}$ gives,
     \begin{align}
  \zeta_{n+\frac{3}{2}} & \Big[ 1-   3 \mathcal{H}_{n+1}  w   \Delta \tau   /2   - (1-c_s^2)   \nabla^2 {\pi_{n+1} }   \Delta \tau   /2 \Big] =\zeta_{n+\frac{1}{2}} +  \Delta \tau \Bigg [   3 \mathcal{H}_{n+1} ( w \frac{\zeta_{n+\frac{1}{2}}}{2} +c_s^2 \Psi_{n+1}) -c_s^2(   3 \mathcal{H}^2_{n+1} 
...
+c_s^2 \nabla^2 \pi_{n+1} 
  ...
  -3 c_s^2 \mathcal{H}_{n+1} (1+w)\pi_{n+1} {\nabla^2 \pi_{n+1} }
      %////////////////
        +  (1-c_s^2)  { \Big( \frac{\zeta_{n+1/2} }{2}+ \Psi_{n+1} \Big) } \nabla^2 {\pi_{n+1} }
                       \nonumber
   \\
    &
    %//////////////// 
 ...
 -\frac{\mathcal{H}_{n+1}} {2 } \Big(2+3w+c_s^2  \Big){\nabla  \pi_{n+1} . \nabla \pi_{n+1}} 
    ...
    Finally we can write the below equation,
      \begin{align}
  \zeta_{n+\frac{3}{2}} &  =\frac{1}{1-   3 \mathcal{H}_{n+1}  w   \Delta \tau   /2   - (1-c_s^2)   \nabla^2 {\pi_{n+1} }   \Delta \tau   /2 }  \Bigg[ \zeta_{n+\frac{1}{2}} +  \Delta \tau \Bigg (   3 \mathcal{H}_{n+1} ( w \frac{\zeta_{n+\frac{1}{2}}}{2} +c_s^2 \Psi_{n+1}) -c_s^2(   3 \mathcal{H}^2_{n+1} 
  - 3 \mathcal{H}' _{n+1}) \pi_{n+1}+ 3 c_s^2 \Phi'_{n+1}  
      \nonumber \\ &
+c_s^2 \nabla^2 \pi_{n+1} 
  % Second order terms
 ...
 -\frac{\mathcal{H}_{n+1}} {2 } \Big(2+3w+c_s^2  \Big){\nabla  \pi_{n+1} . \nabla \pi_{n+1}} 
    %//////////////// 
    \nonumber \\ & 
     +2   (1-c_s^2)\nabla  \pi_{n+1} . {  \nabla {  \big({\zeta_{n+1/2} + \Psi_{n+1} \big)   } }}    \Bigg)   \Bigg]
    \end{align}
...
Then we go through the procedure which is the same for all loops (first loop included), which we update $\pi$ to have it at step 1 and then we update $\zeta$ to have it at step $3/2$ during all of these procedures, $\Phi$ is assumed to be constant and we use $\Phi^{(1/2)}= \Phi^{(0)}$ since in the first loop $\Phi'$ is zero! So this is an approximation that in the first loop we take $\Psi'=0 $ and use the same $\Phi$ at 1/2 and 0 step the same. But in the other loops everything seems correct as following,\\
...
3- The value of $\Phi^{(n+1)} = \Phi^{(n)} + \Phi '^{(n)} \times d \tau  $ to compute $\zeta^{n+3/2}$ which again at the first loop we assume $\Phi^{(1)} = \Phi^{(0) }$\\
...
Another discussion with Julian is,
 ...
 \end{figure}
	{\Large{No Still I have a problem!}}\\to update $\zeta_{int} (n+1)$ from $\zeta(n)$ and $\zeta'(n+1/2)$ we need $\pi(n+1/2)$, $\nabla \pi(n+1/2) $ and etc, which is a big mess!
	 Because  
      \subsection{Non-linear terms tests: {\color{red} How?}}
...
cycle 90, background information: z = 1.85694, average T00 = 0.312045, background model = 0.312046
 ...
 numerator: 5 The rel diff: 1.45638e-05
  ...
  \subsection{Non-linear contribution in the code}
